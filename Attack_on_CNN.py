...
Attack_X = train_X[10000:20000]
Attack_y = train_y[10000:20000]
...
def plotting_a_table_of_number_of_images_per_section():
    table_data = [['Train X', train_X.shape], ['Train y', train_y.shape],
                  ...
                  ['Attack X', Attack_X.shape], ['Attack y', Attack_y.shape]]
...
adversary = generate_adversary(my_model, input_image, input_label, eps=0.1)
EPS_one = generate_adversary(my_model, input_image, input_label, eps=0.01)
EPS_two = generate_adversary(my_model, input_image, input_label, eps=0.15)

pred = my_model.evaluate(adversary, y=Attack_y, batch_size=batch_size)
print(str(pred))

adversary: float = adversary * 255

plt.subplot(2, 2, 1)
plt.imshow(Attack_X[1])
plt.title("Original Image")


plt.subplot(2, 2, 2)
plt.imshow(EPS_one[1])
plt.title("Image after the attack at eps = 0.01")

plt.subplot(2, 2, 3)
plt.imshow(adversary[1])
plt.title("Image after the attack at eps = 0.1")

plt.subplot(2, 2, 4)
plt.imshow(EPS_two[1])
plt.title("Image after the attack at eps = 0.15")

plt.show()

#################################### Confusion Mtrix ###################################################################
pred = my_model.predict(EPS_two)
